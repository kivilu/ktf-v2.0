...
public class SM4 {
    ...
    private byte[] padding( byte[] input, SM4Ctx ctx ) {
        ...
        switch (ctx.padding) {
            case NoPadding:
                ...
                break;
            case ZeroPadding:
                ret = zeroPadding(input, ctx);
                break;
            case PKCS7Padding:
                ret = pkcs7Padding(input, ctx);
                break;
    ...
    private byte[] zeroPadding( byte[] input, SM4Ctx ctx ) {
        byte[] ret = null;
        if (ctx.mode == SM4_ENCRYPT) {
            int p = ctx.blockSize - input.length % ctx.blockSize;
            ByteBuffer buff = ByteBuffer.allocate(input.length + p);
            buff.put(input);
            ret = buff.array();
        }
        else {
            int p = 0;
            for (p = 0; p < ctx.blockSize; p++) {
                if (input[input.length - 1 - p] != 0x0)
                    break;
            }
    ...
    private byte[] pkcs7Padding( byte[] input, SM4Ctx ctx ) {
        byte[] ret = null;
        if (ctx.mode == SM4_ENCRYPT) {
            int p = ctx.blockSize - input.length % ctx.blockSize;
            ret = new byte[input.length + p];
    ...
    public void sm4_setkey_enc( SM4Ctx ctx, byte[] key ) throws Exception {
        if (ctx == null) {
    ...
    public void sm4_setkey_dec( SM4Ctx ctx, byte[] key ) throws Exception {
        if (ctx == null) {
            throw new Exception("ctx is null!");
        }
    ...
    public byte[] sm4_crypt_ecb( SM4Ctx ctx, byte[] input ) throws Exception {
        if (input == null) {
            throw new Exception("input is null!");
        }

        if (ctx.mode == SM4_ENCRYPT && ctx.padding.isPadding())
            input = padding(input, ctx);

        ByteBuffer inbuf = ByteBuffer.wrap(input);
        ByteBuffer outbuf = ByteBuffer.allocate(input.length);
        byte[] in = new byte[16];
        ...
        if (ctx.mode == SM4_DECRYPT && ctx.padding.isPadding())
            output = padding(output, ctx);
        return output;
    }

    public byte[] sm4_crypt_cbc( SM4Ctx ctx, byte[] iv, byte[] input ) throws Exception {
        ...
        if (ctx.mode == SM4_ENCRYPT) {
            while (inbuf.hasRemaining()) {
                ...
                for (i = 0; i < 16; i++) {
                    xor[i] = ((byte) (in[i] ^ xor[i]));
                }
                sm4_one_round(ctx.sk, xor, out);
                System.arraycopy(out, 0, xor, 0, 16);
                outbuf.put(out);
            }
        }
        else {
            while (inbuf.hasRemaining()) {
                ...
                sm4_one_round(ctx.sk, in, out);
                for (i = 0; i < 16; i++) {
                    out[i] = ((byte) (out[i] ^ xor[i]));
                }
                System.arraycopy(in, 0, xor, 0, 16);
                outbuf.put(out);
            }
        }
