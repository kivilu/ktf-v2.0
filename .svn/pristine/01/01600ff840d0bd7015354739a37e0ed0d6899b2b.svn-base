...
public class RedisLockKit {
	 ...
	 * Acquires the lock. If the lock is not available then the current thread
	 * becomes disabled for thread scheduling purposes and lies dormant until the
	 * lock has been acquired. If the lock is acquired, it is held until unlock is
	 * invoked, or until leaseTime milliseconds have passed since the lock was
	 ...
	 * false. A typical usage idiom for this method would be: Lock lock = ...; if
	 * (lock.tryLock()) { try { // manipulate protected state } finally {
	 * lock.unlock(); } } else { // perform alternative actions } This usage ensures
	 * that the lock is unlocked if it was acquired, and doesn't try to unlock if
	 ...
	 * thread has not been interrupted. If the lock is available this method returns
	 * immediately with the value true. If the lock is not available then the
	 ...
	 * the current thread, and interruption of lock acquisition is supported; or The
	 * specified waiting time elapses
	 * 
	 * If the lock is acquired then the value true is returned.
	 * 
	 * If the current thread: has its interrupted status set on entry to this
	 * method; or is interrupted while acquiring the lock, and interruption of lock
	 * acquisition is supported, then InterruptedException is thrown and the current
	 * thread's interrupted status is cleared. If the specified waiting time elapses
	 * then the value false is returned. If the time is less than or equal to zero,
	...
	/**
	 * Returns true as soon as the lock is acquired. If the lock is currently held
	 * by another thread in this or any other process in the distributed system this
	 * method keeps trying to acquire the lock for up to waitTime before giving up
	 * and returning false. If the lock is acquired, it is held until unlock is
	 * invoked, or until leaseTime have passed since the lock was granted -
