...
public class JwtRealm extends AuthorizingRealm {
    ...
    protected AuthenticationInfo doGetAuthenticationInfo(AuthenticationToken token) throws AuthenticationException {
        ...
        try {
            ...
            authenticationInfo = new SimpleAuthenticationInfo(su, accessToken, getName());

        } catch (KtfException e) {
            throw new IncorrectCredentialsException(e.getTips());
        } catch (Exception e) {
            log.error("shiro验证失败", e);
            throw new IncorrectCredentialsException("shiro验证失败");
        }
     ...
     * <p>
     * 权限信息.(授权): 1、如果用户正常退出，缓存自动清空； 2、如果用户非正常退出，缓存自动清空； 3、如果我们修改了用户的权限，而用户不退出系统，修改的权限无法立即生效。 （需要手动编程进行实现；放在service进行调用）
     * 在权限修改后调用realm中的方法，realm已经由spring管理，所以从spring中获取realm实例，调用clearCached方法； Authorization
    ...
    protected AuthorizationInfo doGetAuthorizationInfo(PrincipalCollection principals) {
        ...
        ShiroUser shiroUser = JSON.parseObject(principals.getPrimaryPrincipal().toString(), ShiroUser.class);
