...
public class CifCustomerAuthsServiceImpl extends ServiceImpl<CifCustomerAuthsMapper, CifCustomerAuths>
    implements ICifCustomerAuthsService {

    @Autowired
    @Qualifier("cifAuthentication")
    ...
    public Long save(CifCustomerAuthsDTO dto) {
        String password = StrKit.isBlank(dto.getCredential())
            ? DigestUtil.hashBase64(AlgDigest.SM3, cifProperties.getDefaultPassword()) : dto.getCredential();

        String salt = StrKit.isBlank(dto.getCredentialSalt()) ? StrKit.random(16) : dto.getCredentialSalt();
