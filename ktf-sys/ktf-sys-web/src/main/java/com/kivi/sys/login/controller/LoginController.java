package com.kivi.sys.login.controller;
...
import com.kivi.sys.shiro.ktf.KtfShiroToken;
...
public class LoginController extends DashboardController {
    ...
    @KtfTrace("登录选项")
    @ApiOperation(value = "登录选项", notes = "登录选项")
    ...
    public ResultMap nonce(@ApiIgnore HttpSession session) {
        ...
        return ResultMap.ok().data(nonce);
    }
    ...
    public ResultMap login(@Valid @RequestBody LoginForm form, @ApiIgnore HttpSession session) throws Exception {
        ...
        KtfShiroToken token = new KtfShiroToken(form);
        subject.login(token);
        ...
        // 保存登录时间和IP
        CifCustomerAuthsDTO customerAuths = new CifCustomerAuthsDTO();
    ...
    public ResultMap createToken(UserVo userVo) throws Exception {
        // 创建Jwt Toen
        String jwtToken = ShiroUserKit.me().generateJwtToken(userVo);
    ...
    class SaveRunnable implements Runnable {
        private final CifCustomerAuthsDTO customerAuths;

        public SaveRunnable(CifCustomerAuthsDTO customerAuths) {
