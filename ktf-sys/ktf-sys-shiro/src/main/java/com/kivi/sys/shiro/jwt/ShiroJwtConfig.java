...
import com.kivi.sys.shiro.configure.ShiroRedisConfig;
import com.kivi.sys.shiro.ktf.AnyRolesAuthorizationFilter;
import com.kivi.sys.shiro.ktf.KtfCredentialsMatcher;
import com.kivi.sys.shiro.ktf.KtfModularRealmAuthenticator;
import com.kivi.sys.shiro.ktf.KtfModularRealmAuthenticatorWithoutRpc;
import com.kivi.sys.shiro.ktf.KtfPermsFilter;
import com.kivi.sys.shiro.ktf.KtfRealm;
 ...
 *
 */
@ConditionalOnProperty(prefix = KtfSysProperties.PREFIX, value = "auth", havingValue = "jwt", matchIfMissing = false)
@AutoConfigureAfter({SpringContextHolder.class, ShiroRedisConfig.class, SysKit.class})
@Configuration
public class ShiroJwtConfig extends ShiroBaseConfig {
    ...
    @ConditionalOnProperty(prefix = KtfCommonProperties.PREFIX, name = {"enable-dubbo"}, havingValue = "false",
        matchIfMissing = true)
    @Bean
    public Authenticator authenticatorWithoutRpc() {
        KtfModularRealmAuthenticatorWithoutRpc authenticator = new KtfModularRealmAuthenticatorWithoutRpc();
        authenticator.setRealms(Arrays.asList(jwtShiroRealm(), ktfRealm()));
        authenticator.setAuthenticationStrategy(new FirstSuccessfulStrategy());
        return authenticator;
    }
    ...
    @ConditionalOnProperty(prefix = KtfCommonProperties.PREFIX, name = {"enable-dubbo"}, havingValue = "false",
        matchIfMissing = true)
    @Bean
    public DefaultWebSecurityManager securityManagerWithoutRpc() {
        DefaultWebSecurityManager securityManager = new DefaultWebSecurityManager();
        // 设置realm.
        securityManager.setRealms(Arrays.asList(ktfRealm(), jwtShiroRealm()));
        securityManager.setAuthenticator(authenticatorWithoutRpc());
        // 注入Session管理器
        // securityManager.setSessionManager(sessionManager());
        // 注入缓存管理器
        // securityManager.setCacheManager(redisCacheManager);
    ...
    public Realm ktfRealm() {
        KtfRealm myShiroRealm = new KtfRealm(ktfTokenService, ktfCredentialsMatcher());
    ...
    protected KtfCredentialsMatcher ktfCredentialsMatcher() {
        ...
        return new KtfCredentialsMatcher();
    }
