...
public class SysRegionServiceImpl extends ServiceImpl<SysRegionMapper, SysRegion> implements SysRegionService {
    ...
    public SysRegionDTO getDTOById(Long id) {
        ...
        SysRegionDTO dto =
            BeanConverter.convert(SysRegionDTO.class, entity, BeanConverter.long2String, BeanConverter.integer2String);
        return dto;
    }
    ...
    public List<SysRegionDTO> listProvice() {
        QueryWrapper<SysRegion> query = Wrappers.<SysRegion>query()
            .select(SysRegion.DB_ID, SysRegion.DB_PID, SysRegion.CODE, SysRegion.DB_NAME).orderByAsc(SysRegion.CODE);
    ...
    public Boolean removeWithChildren(Long id) {
        ...
        wrapper.eq(SysRegion.DB_PID, id);
        this.remove(wrapper);
        return this.removeById(id);
    }

    @Override
    public Boolean removeWithChildren(List<Long> ids) {
        QueryWrapper<SysRegion> wrapper = new QueryWrapper<>();
        wrapper.in(SysRegion.DB_PID, ids);
        this.remove(wrapper);
