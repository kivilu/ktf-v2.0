...
import com.kivi.framework.crypto.service.AsymCryptoService;
...
public abstract class BaseAsymCryptoService<T> implements AsymCryptoService {
    ...
    protected Signature signature( String signAlg ) {
        ...
        if (signatureMap.containsKey(signAlg)) {
            signature = signatureMap.get(signAlg).get();
        }
        else {
            ThreadLocal<Signature> theadLocal = new ThreadLocal<Signature>() {
                @Override
                protected Signature initialValue() {
                    return CipherUtil.getSignatureInstance(signAlg);
                }

            };
            signatureMap.put(signAlg, theadLocal);
            signature = theadLocal.get();
        }
