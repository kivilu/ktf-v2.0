...
import com.kivi.sys.sys.entity.SysSmsRecordEx;
import com.kivi.sys.sys.entity.UnreadSms;
...
public class HomeSmsPushSocket {
	...
	public void onOpen(Session session, @PathParam("userId") String userId) {
		try {
			...
			// 连接成功，发送消息
			Long					count			= smsRecordService.findUnreadMessagesCount(Long.valueOf(tempId));
			List<SysSmsRecordEx>	smsRecordList	= smsRecordService.findRecent5Messages(Long.valueOf(tempId));
	...
	public void onMessage(String message, Session session) {
		...
		try {
			...
			String[]	targetList	= StringUtils.split(",");
			Long[]		userIds		= StrKit.toLongArray(targetList);

			smsRecordService.updateMessageStatus(userIds);
			Long					count			= smsRecordService.findUnreadMessagesCount(Long.valueOf(tempId));
			List<SysSmsRecordEx>	smsRecordList	= smsRecordService.findRecent5Messages(Long.valueOf(tempId));
			UnreadSms				unreadSms		= new UnreadSms();
			unreadSms.setCount(count);
			...
			log.error("SmsPushSocket: webSocket发送消息异常：登录用户：" + tempId, e);
		}
	}
