...
public class AesServiceImpl extends BaseSymmCryptoService implements AesService {
	...
	@Override
	public String ctx(String ctxToken, AlgAES alg, byte[] key, byte[] iv) {
		...
		SymCtx ctx = SymCtx.builder().uuid(ctxToken).zeroPadding(alg.zeroPadding()).alg(algName).cbc(iv != null)
				.key(key).iv(iv).secretKey(new SecretKeySpec(key, "AES")).build();
		super.putCtx(ctx);
		return ctxToken;
	}
