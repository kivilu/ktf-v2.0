		...
		if (request instanceof MultipartHttpServletRequest) {
			...
			while (iterator.hasNext()) {
				...
				if (!checkFile(filename)) {
					log.error("只允许上传gif,jpg,jpeg,bmp,png,jar,doc,docx,xls,xlsx,pdf,txt,rar格式文件");
					new KtfException("只允许上传gif,jpg,jpeg,bmp,png,jar,doc,docx,xls,xlsx,pdf,txt,rar格式文件");
					flag = false;
				}
			}
		}
