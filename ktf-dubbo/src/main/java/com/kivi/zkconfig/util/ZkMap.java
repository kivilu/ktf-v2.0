package com.kivi.zkconfig.util;
...
public class ZkMap implements Map<String, Object>, Closeable {
	private final CuratorFramework	client;
	private final String			connectionString;
	private final String			root;

	public static ZkMap newMap(String connectionString) {
		return newBuilder(connectionString).build();
	}

	public static ZkMap newMap(String connectionString, String root) {
		return newBuilder(connectionString).withRoot(root).build();
	}

	public static ZkMap newMap(CuratorFramework client, String root) {
		return new ZkMap(client, root);
	}
	...
	private ZkMap(Builder builder) {
		...
		if (connectStringParser.getChrootPath() != null) {
			...
			try (final CuratorFramework clientForChrootCreation = newCuratorFrameworkClient(builder,
					connectionStringForChrootCreation)) {
	...
	private ZkMap(CuratorFramework client, String root) {
		this.client				= client;
		this.root				= root;
	...
	private static CuratorFramework newCuratorFrameworkClient(Builder builder, String connectionString) {
		return CuratorFrameworkFactory.builder().connectString(connectionString).retryPolicy(builder.retryPolicy)
				.connectionTimeoutMs((int) builder.duration.toMillis()).build();
	}
	...
	private Object getValue(String key) {
		return tryIt(() -> {
			...
			return bytes != null ? FstSerializer.deserialize(bytes) : null;
		});
	}
	...
	public Object put(String key, Object value) {
		if (key != null && !key.isEmpty()) {
			...
			tryIt(() -> client.setData().forPath(keyPath(key), value != null ? FstSerializer.serialize(value) : null));
	...
	public Collection<Object> values() {
		return tryIt(() -> getKeys().stream().map(this::getValue).collect(Collectors.toList()));
	}
	...
	private Function<String, Entry<String, Object>> entryFromKey() {
	...
	public final boolean equals(Object o) {
		...
		if (!(o instanceof ZkMap))
			return false;

		ZkMap otherZooMap = (ZkMap) o;
	...
	static class Periscope {
		static CuratorFramework client(ZkMap zooMap) {
