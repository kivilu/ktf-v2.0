...
//	@NacosInjected
	...
	public KtfNacosConfigKit(LoggingSystem loggingSystem, ConfigService configService) {
...
//	}
	 ...
	 * @return
	 */
	public Map<String, Object> getValue(String dataId, String group, String keyPrefix) {
		try {
			String content = configService.getConfig(dataId, group, 5000);
			log.trace("current config: {}", content);
			Map<String, Object> result = new KtfYamlConfigParse().parse(content);
...
//					.collect(Collectors.toMap(Map.Entry::getKey, Map.Entry::getValue, (k1, k2) -> k1));
	...
	public boolean setValue(String dataId, String group, String key, Object value) {
		Map<String, Object> values = CollectionKit.newHashMap(key, value);

		return this.setValue(dataId, group, values);
	}

	@KtfTrace("设置Nacos值")
	public boolean setValue(String dataId, String group, Map<String, Object> values) {
		boolean isPublishOk = false;
		try {
			String content = configService.getConfig(dataId, group, 5000);
			log.trace("current config: {}", content);

			Map<String, Object> map = new KtfYamlConfigParse().parse(content);
			map.putAll(values);
//			Properties curProps = new KtfYamlConfigParse().parse(content);
//			curProps.putAll(values);

			List<Object>	newProps	= map.entrySet().stream().map(ent -> {
	...
	public boolean setValue(String dataId, String group, String key, List<Object> values) {
		Map<String, Object> props = CollectionKit.newHashMap();
		IntStream.range(0, values.size()).forEach(x -> {
			String realKey = StrKit.join("[", x, "]");
			props.put(realKey, values.get(x));
		});
