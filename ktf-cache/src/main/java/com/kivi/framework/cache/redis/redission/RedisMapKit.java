...
public class RedisMapKit {
    ...
    /**
     * Associates the specified value with the specified key. Works faster than RedisMap.put but not returning the
     * previous value associated with key
     * 
     * @param key
     * @param value
     * @return true if key is a new key in the hash and value was set. false if key already exists in the hash and the
     *         value was updated.
     */
     ...
     * removed. (The map can contain at most one such mapping.) Returns the value to which this map previously
     * associated the key, or null if the map contained no mapping for the key.
     * 
    ...
    /**
     * Removes keys from map by one operation Works faster than RMap.remove but not returning the value associated with
     * key
     * 
     * @param keys
     * @return
     ...
     * this map contains a mapping for a key k such that (key==null ? k==null : key.equals(k)). (There can be at most
     * one such mapping.)
     * 
     * @param name
     * @param key
