...
public class RedisMap {
    ...
    public RedisMap( RedissonClient redissonClient ) {
        this(redissonClient, null);
    }

    public RedisMap( RedissonClient redissonClient, Codec codec ) {
        this.redissonClient = redissonClient;
        this.codec = codec;
    }

    public <K, V> RMap<K, V> getRMap( String name ) {
        return codec == null ? redissonClient.getMap(PREFIX + name) : redissonClient.getMap(PREFIX + name, codec);
    }
     ...
     * @param key
     * @param value
     * @return true if key is a new key in the hash and value was set. false if
     *         key already exists in the hash and the value was updated.
     */
    ...
    /**
     * Removes keys from map by one operation Works faster than RMap.remove but
     * not returning the value associated with key
     * 
     * @param keys
     * @return
     */
