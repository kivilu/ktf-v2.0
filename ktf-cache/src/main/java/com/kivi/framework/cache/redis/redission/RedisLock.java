...
public class RedisLock {
    private static final String  PREFIX = "RLOCK_";
    ...
    private RLock getLock( String name ) {
        log.trace("get RLock:{}", name);
        RLock rlock = redissonClient.getLock(PREFIX + name);
        return rlock;
    }
     ...
     * Acquires the lock. If the lock is not available then the current thread
     * becomes disabled for thread scheduling purposes and lies dormant until
     * the lock has been acquired. If the lock is acquired, it is held until
     * unlock is invoked, or until leaseTime milliseconds have passed since the
     ...
     *            by invoking unlock. If leaseTime is -1, hold the lock until
     *            explicitly unlocked.
     * @param unit
     *            the time unit of the leaseTime argument
     */
    ...
    /**
     * Acquires the lock only if it is free at the time of invocation. Acquires
     * the lock if it is available and returns immediately with the value true.
     * If the lock is not available then this method will return immediately
     * with the value false. A typical usage idiom for this method would be:
     * Lock lock = ...; if (lock.tryLock()) { try { // manipulate protected
     * state } finally { lock.unlock(); } } else { // perform alternative
     * actions } This usage ensures that the lock is unlocked if it was
     * acquired, and doesn't try to unlock if the lock was not acquired.
     * 
     ...
     * method returns immediately with the value true. If the lock is not
     ...
     * method; or is interrupted while acquiring the lock, and interruption of
     * lock acquisition is supported, then InterruptedException is thrown and
     * the current thread's interrupted status is cleared. If the specified
    ...
    /**
     * Returns true as soon as the lock is acquired. If the lock is currently
     * held by another thread in this or any other process in the distributed
     * system this method keeps trying to acquire the lock for up to waitTime
     * before giving up and returning false. If the lock is acquired, it is held
     * until unlock is invoked, or until leaseTime have passed since the lock
     * was granted - whichever comes first.
     * 
     * @param waitTime
